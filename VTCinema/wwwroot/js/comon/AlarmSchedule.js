...
function GetAlarmSchedule(exdays, second_alram, empid) {
    $.ajax({
        url: "/Views/Todo/AlarmSchedule/pageMyAlarmScheduler_List.aspx/LoadAlarmScheduleToday",
        dataType: "json",
        type: "POST",
        data: JSON.stringify({}),
        contentType: 'application/json; charset=utf-8',
        async: false,
        error: function (XMLHttpRequest, textPrint, errorThrown) {
        },
    ...
    for (i = 0; i < data.length; i++) {
        ...
        if (numSecond < 0) {
            ...
            if (numSecond < second_alram) {
                notiAlarmSchedule(item.Title, item.Content);
                DeleteItemAlarmCookie(item.ID, 1);
            }
        }
    }
