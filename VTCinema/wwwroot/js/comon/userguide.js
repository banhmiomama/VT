...
function FistSlideGuide() {
    ...
    $("#divFistSlideGuide").load("/Views/Guide/pageFirstGuideSlide.aspx" + "?ver=" + versionofWebApplication);
    $('#divFistSlideGuide').modal('show');
}
function DocumentGuideUser() {
    var win = window.open("/Views/Guide/pageDocumentation.aspx" + "?ver=" + versionofWebApplication, '_blank');
    win.focus();
}
function RunGuideEndUser(_guide) {
    ...
    if (_GuideForCurrentPage == '[]') $('#btnGuideEndUser').hide();
        ...
        let obj_nextButton = { className: "myNext", text: "tiep theo" };
        let obj_skipButton = { className: "mySkip", text: "OK xong" };
